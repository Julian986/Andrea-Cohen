  ...
  return (
    <header className="fixed top-0 inset-x-0 z-50 bg-white/80 backdrop-blur border-b border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="h-16 flex items-center justify-between md:justify-center">
          <a href="#inicio" className="font-semibold text-slate-800 text-lg tracking-tight md:absolute md:left-4 lg:left-8">Lic. Andrea <span className="text-emerald-600">Cohen</span></a>

          <nav className="hidden md:flex items-center gap-8 text-slate-700">
            {navItems.map(item => (
              <a key={item.href} href={item.href} className="hover:text-emerald-600 transition-colors font-medium">{item.label}</a>
            ))}
          </nav>
      ...
      {open && (
        <div className="md:hidden fixed inset-0 z-[9999] bg-white h-dvh min-h-screen overflow-auto animate-fadeIn">
          <div className="h-16 px-4 flex items-center justify-between border-b border-slate-200">
            <a href="#inicio" onClick={() => setOpen(false)} className="font-semibold text-slate-800 text-lg tracking-tight">Lic. Andrea <span className="text-emerald-600">Cohen</span></a>
          ...
          <nav className="px-6 py-4 grid text-lg divide-y divide-slate-200">
            {navItems.map(item => (
              <a key={item.href} href={item.href} onClick={() => setOpen(false)} className="py-4 text-slate-700 hover:text-emerald-600 transition-colors">{item.label}</a>
            ))}
          </nav>
        </div>
      )}
